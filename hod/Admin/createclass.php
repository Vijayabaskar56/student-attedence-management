<?php
    ...
    // -------------------------EDIT------------------------
    if (isset($_GET['classId']) && isset($_GET['action']) && $_GET['action'] == "edit") {
    {
        $Id=$_GET['classId'];
        $query= mysqli_query($conn,"SELECT * from tblclass where classId = '$Id'");
        $row = mysqli_fetch_array($query);

        // -------------UPDATE--------------
        if(isset($_POST['update'])){
            $className=$_POST['className'];
            $query=mysqli_query($conn,"UPDATE tblclass set className='$className' where classId='$Id'");
    ...
    // -------------------------DELETE-------------------------
    if(isset($_GET['classId']) && isset($_GET['action']) && $_GET['action'] == "delete")
    {
        $Id = $_GET['classId'];
        $query = mysqli_query($conn,"DELETE FROM tblclass WHERE classId='$Id'");
                            ...
                            <?php
                                ...
                                if($num > 0)
                                {
                                    while ($rows = $rs->fetch_assoc()) {
                                        ...
                                        <td><a href='?action=edit&classId=".$rows['classId']."'><i class='bx bxs-edit'></i>Edit</a></td>
                                        <td><a href='?action=delete&classId=".$rows['classId']."'><i class='bx bxs-trash-alt'></i>Delete</a></td>
                                        </tr>
                                        ";
                                    }
                                }
